...
*===============================================================================
{
	* Parallel operationalization, do not modify
		...
		local iniyear=2022	// Do not modify
		local finyear=2022	// Do not modify
			...
			* Create n batch_`bi'.do files
			forval bi = `iniparallelyear'/`finparallelyear' {
				...
				* Replace xrxx and xryy with initial and final years
				!sed -i '' "s/iniyear=2022/iniyear=`bi'/g" batch_`bi'.do
				!sed -i '' "s/finyear=2022/finyear=`bi'/g" batch_`bi'.do
		...
		if "`parallel'"=="yes" & c(os)=="Windows" {
			...
			* Create n batch_`bi'.do files
			forval bi = `iniparallelyear'/`finparallelyear' {
				...
				*!powershell -command " (Get-Content batch_`bi'.do) -replace 'iniyear=2022', 'iniyear=`bi'' | Out-File -encoding ASCII batch_`bi'.do "
				*!powershell -command " (Get-Content batch_`bi'.do) -replace 'finyear=2022', 'finyear=`bi'' | Out-File -encoding ASCII batch_`bi'.do "
				* Turn off parallel option
				*!powershell -command " (Get-Content batch_`bi'.do) -replace '*local parallel', '**local parallel' | Out-File -encoding ASCII batch_`bi'.do "
				
				!powershell -command " (Get-Content batch_`bi'.do) -replace 'iniyear=2022', 'iniyear=`bi'' | Out-File -encoding ASCII batch_`bi'.do ; (Get-Content batch_`bi'.do) -replace 'finyear=2022', 'finyear=`bi'' | Out-File -encoding ASCII batch_`bi'.do ; (Get-Content batch_`bi'.do) -replace '*local parallel', '**local parallel' | Out-File -encoding ASCII batch_`bi'.do"
