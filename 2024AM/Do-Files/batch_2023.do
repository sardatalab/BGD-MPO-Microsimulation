			...
			* Create n batch_`bi'.do files
			forval bi = `iniparallelyear'/`finparallelyear' {
				...
				*!powershell -command " (Get-Content batch_`bi'.do) -replace '*local parallel', '**local parallel' | Out-File -encoding ASCII batch_`bi'.do "
				...
				!powershell -command " (Get-Content batch_`bi'.do) -replace 'iniyear=2023', 'iniyear=`bi'' | Out-File -encoding ASCII batch_`bi'.do ; (Get-Content batch_`bi'.do) -replace 'finyear=2023', 'finyear=`bi'' | Out-File -encoding ASCII batch_`bi'.do ; (Get-Content batch_`bi'.do) -replace '*local parallel', '**local parallel' | Out-File -encoding ASCII batch_`bi'.do"
