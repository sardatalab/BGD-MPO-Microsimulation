			...
			* Create n batch_`bi'.do files
			forval bi = `iniparallelyear'/`finparallelyear' {
				...
				* Replace xrxx and xryy with initial and final years
				!sed -i '' "s/iniyear=2027/iniyear=`bi'/g" batch_`bi'.do
				!sed -i '' "s/finyear=2027/finyear=`bi'/g" batch_`bi'.do
			...
			* Create n batch_`bi'.do files
			forval bi = `iniparallelyear'/`finparallelyear' {
				...
				* Replace _fake xrxx and xryy with initial and final years
				!powershell -command " (Get-Content batch_`bi'.do) -replace 'iniyear=2027', 'iniyear=`bi'' | Out-File -encoding ASCII batch_`bi'.do "
				!powershell -command " (Get-Content batch_`bi'.do) -replace 'finyear=2027', 'finyear=`bi'' | Out-File -encoding ASCII batch_`bi'.do "
				* Turn off parallel option
